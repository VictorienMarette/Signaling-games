        ...
        for subset in subsets:
            ...
            if result.success:
                ...
                for tsubset in Tsubsets:
                    ...
                    b_eq_T_subset = np.copy(b_eq)
                    for i in tsubset:
                        addedligne = np.zeros((1,len_T))
                        addedligne[0][i] = 1
                        A_eq_T_subset = np.vstack((A_eq_T_subset,addedligne))
                        b_eq_T_subset = np.append(b_eq_T_subset, 0)

                    
                    result = linprog(c, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq_T_subset , b_eq=b_eq_T_subset , bounds=(0, None))
                    if result.success:
