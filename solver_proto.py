...
def solve_PBE_frac(T,S,A,U,U_r):
    ...
    geometry_corresponding_to_indifferent_actions = {}
    coordinates_symboles = [sp.symbols("x"+str(i)) for i in range(len(T))]
    linear_combinaison_symbole = {}
    for s in indifferent_actions:
        pass



    for s in indifferent_actions:
        geometry_corresponding_to_indifferent_actions[s] = {}
        for actions in indifferent_actions[s]:
            geometry_corresponding_to_indifferent_actions[s][actions] = {}
            for states in indifferent_actions[s][actions]:
                geometry_corresponding_to_indifferent_actions[s][actions][states] = [[],[],[]]
                
                if states == ():
                    if len(actions) == 1:
                        geometry_corresponding_to_indifferent_actions[s][actions][states].append
